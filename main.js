...
function visualmode() {
  ...
  // Correctly check mode *after* toggling:
  var isLight = document.body.classList.contains("light-mode");
  document.querySelectorAll(".needtobeinvert").forEach(function (e) {
    if (isLight) {
      e.classList.add("invertapplied"); // black icon in light mode
    } else {
      e.classList.remove("invertapplied");    // white icon in dark mode
    }
  });
}
...
window.addEventListener("DOMContentLoaded", function() {
  ...
  document.querySelectorAll(".needtobeinvert").forEach(function (e) {
    if (isLight) {
      ...
      e.classList.remove("invertapplied");
    }
  });
  ...
  // 3. Open the settings panel and its buttons
  const settingContainer = document.getElementById("setting-container");
  if (settingContainer) settingContainer.classList.add("settingactivate");
  const visualModeBtn = document.getElementById("visualmodetogglebuttoncontainer");
  if (visualModeBtn) visualModeBtn.classList.add("visualmodeshow");
  const soundToggleBtn = document.getElementById("soundtogglebuttoncontainer");
  if (soundToggleBtn) soundToggleBtn.classList.add("soundmodeshow");
});
